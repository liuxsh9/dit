...
class SidecarEntry:
    ...
    char_count: int                  # total character count of the row JSON
    ...
    created_at: str                  # ISO 8601 timestamp of when this entry was computed
...
```
.datahub/objects/sidecars/<hash[:2]>/<hash>    (zstd compressed)
```
...
2. Parse as JSON (it's a single JSONL line, canonical JSON)
3. Compute:
   - `char_count = len(row_text)` (byte length of the original row)
...
```
GET /meta/{commit_hash}/{file_path}
...
```
GET /meta/{commit_hash}/{file_path}/summary
