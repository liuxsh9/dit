...
本系列指南用于对 DataHub（`dit`）的**全部功能模块**进行人工 QA 验收测试。测试者按照指南编号从 00 开始逐步执行，每节均提供明确的操作步骤、预期输出和验证命令，确保功能行为与设计规格一致。

---

## 概述

DataHub 是一个面向机器学习训练数据的版本管理系统，提供类 Git 的本地 CLI（`dit`）、基于 HTTP 的服务端 REST API、元数据计算、导出/搜索/去重等运维功能。
...
**代码仓库**：测试前须已克隆 DataHub 源码并位于项目根目录。

**数据库**：需要一个可用的 PostgreSQL 实例，测试过程中会创建专用数据库（如 `datahub_test`），测试结束后可手动删除。
